    ...
    public static void setCustomJPackageToolProvider() {
        JPackageCommand.useToolProviderByDefault(new ToolProvider() {
                ...
                return parse(out, err, args).peekErrors(errors -> {
                    ...
                    return new OptionsProcessor(builder, new CliBundlingEnvironment() {
                        ...
                        public Collection<BundlingOperationDescriptor> supportedOperations() {
                            return List.of(defaultOperation().orElseThrow());
                        }
            ...
            private Result<JOptSimpleOptionsBuilder.OptionsBuilder> parse(PrintWriter out, PrintWriter err, String... args) {
                try {
                    final var builder = new JOptSimpleOptionsBuilder().unrecognizedOptionHandler(optionName -> {
                        return I18N.buildConfigException("ERR_InvalidOption",
                                new OptionName(optionName).formatForCommandLine()).create();
...
//                "--jpt-include=ErrorTest.test(NATIVE; args-add=[--runtime-image, @@JAVA_HOME@@, --app-image, app-image]; errors=[ERR_MutuallyExclusiveOptions+[--runtime-image, --app-image]])",
