...
public final class JUnitUtils {
    ...
    private final static ObjectMapper EXCEPTION_OM = ObjectMapper.standard()
            .mutate(builder -> {
                builder.methods(Exception.class).exclude("getMessage", "getCause").apply();
            }).mutator(Exception.class, (ex, map) -> {
                var eit = map.entrySet().iterator();
                while (eit.hasNext()) {
                    var e = eit.next();
                    if (e.getValue() == ObjectMapper.NULL) {
