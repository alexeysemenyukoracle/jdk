...
public final class StandardOption {
    ...
    public static final OptionValue<Boolean> HELP = auxilaryOption("help").shortName("h").create();

    public static final OptionValue<Boolean> VERSION = auxilaryOption("version").create();

    public static final OptionValue<Boolean> VERBOSE = auxilaryOption("verbose").create();

    public static final OptionValue<PackageType> TYPE = option("type", PackageType.class).shortName("t")
    ...
    public static final OptionValue<Path> INPUT = directoryOption("input").shortName("i")
            .outOfScope(NOT_BUILDING_APP_IMAGE)
            .create();

    public static final OptionValue<Path> DEST = directoryOption("dest").shortName("d")
    ...
    public static final OptionValue<String> VENDOR = stringOption("vendor").create();
    ...
    public static final OptionValue<String> NAME = stringOption("name").shortName("n")
    ...
    public static final OptionValue<String> COPYRIGHT = stringOption("copyright").create();
    ...
    static final OptionValue<List<Path>> FILE_ASSOCIATIONS = fileOption("file-associations")
            .toArray(pathSeparator())
            .outOfScope(BundlingOperationModifier.BUNDLE_RUNTIME)
            .create(toList());

    static final OptionValue<List<AdditionalLauncher>> ADD_LAUNCHER = createAddLauncherOption("add-launcher");
    ...
    public static final OptionValue<String> MODULE = stringOption("module").shortName("m")
            .valuePattern("module-name[/main-class]")
            .outOfScope(NOT_BUILDING_APP_IMAGE)
            .mutate(launcherProperty())
            .create();

    public static final OptionValue<List<String>> ADD_MODULES = stringOption("add-modules").toArray(",")
            .valuePattern("module-name")
            .outOfScope(NOT_BUILDING_APP_IMAGE)
            .create(toList());

    public static final OptionValue<List<Path>> MODULE_PATH = pathOption("module-path").toArray(pathSeparator())
            .valuePattern("module-name")
            .outOfScope(NOT_BUILDING_APP_IMAGE)
            .create(toList());
    ...
    public static final OptionValue<String> LINUX_BUNDLE_NAME = stringOption("linux-package-name").scope(nativeBundling()).create();

    public static final OptionValue<String> LINUX_DEB_MAINTAINER = stringOption("linux-deb-maintainer")
            .valuePattern("email-address")
            .create();

    public static final OptionValue<String> LINUX_CATEGORY = stringOption("linux-app-category").scope(nativeBundling()).create();
    ...
    public static final OptionValue<String> LINUX_PACKAGE_DEPENDENCIES = stringOption("linux-package-deps").scope(nativeBundling()).create();

    public static final OptionValue<Boolean> LINUX_SHORTCUT_HINT = booleanOption("linux-shortcut")
            .scope(nativeBundling())
            .mutate(launcherProperty())
            .create();
    ...
    public static final OptionValue<Boolean> MAC_SIGN = booleanOption("mac-sign").scope(MAC_SIGNING).shortName("s").create();
    ...
    public static final OptionValue<Path> MAC_ENTITLEMENTS = fileOption("mac-entitlements").scope(MAC_SIGNING).create();
    ...
    public static final OptionValue<Boolean> WIN_MENU_HINT = booleanOption("win-menu")
            .scope(nativeBundling())
            .mutate(launcherProperty())
            .create();
    ...
    public static final OptionValue<Boolean> WIN_SHORTCUT_HINT = booleanOption("win-shortcut")
            .scope(nativeBundling())
            .mutate(launcherProperty())
            .create();
     ...
     */
    public static final OptionValue<List<Options>> FA = OptionValue.create();
    ...
    private static OptionSpecBuilder<String>.ArrayOptionSpecBuilder escapedStringListOption(String name) {
        return stringOption(name).toArray(str -> {
...
//                                optionValue, "error.paramater-add-launcher-malformed", cause);
                    ...
                    if (!StandardValidator.IS_NAME_VALID.test(launcherName)) {
                        throw new AddLauncherIllegalNameException(I18N.format("ERR_InvalidSLName", launcherName));
                    }
    ...
    private static final class AddLauncherIllegalNameException extends IllegalArgumentException implements KnownExceptionType {

        AddLauncherIllegalNameException(String message) {
            super(message);
        }
    ...
    private static final class AddLauncherSyntaxException extends IllegalArgumentException implements KnownExceptionType {
    ...
    private static final class AddLauncherInvalidPropertyFile extends IllegalArgumentException implements KnownExceptionType {
    ...
    private record LauncherScalarProperty<T>(OptionSpecBuilder<T> builder) implements LauncherProperty<T> {
        ...
        public OptionSpec<T> optionSpecForPropertyFile() {
            var optionSpec = builder.createOptionSpec();
            if (optionSpec.hasValue()) {
                return optionSpec;
            } else if(builder.valueType().equals(Boolean.class)) {
                // Boolean option which is set to "true" if the option is on the command line
                // and to "false" otherwise.
                // This doesn't work for properties in a property file as "foo=true" and
                // "foo=false" will be interpreted as "true" because they are present on a
                // synthesized command line.
                // Need explicit String -> boolean converter.
                return (OptionSpec<T>)((OptionSpecBuilder<Boolean>)builder.copy()).converter(
                        StandardValueConverter.booleanConv()).createOptionSpec();
            } else {
                // If the option spec doesn't have a value it should be of Boolean type, but it is not.
