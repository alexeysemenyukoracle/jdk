...
/**
 * Summary of an operation jpackage will perform.
 */
public final class Summary implements SummaryAccumulator {

    public void print(Consumer<String> sink) {
        ...
        warnings.entrySet().stream().sorted(comparator()).map(e -> {
            return I18N.format("summary.warning", e.getValue());
        }).forEach(sink);
    }
    ...
    public void putIfAbsent(SummaryItem k, String value) {
        ...
        switch (k) {
            ...
            case Warning warn -> {
                warnings.putIfAbsent(wrapIdentity(warn), value);
            }
        }
    }
    ...
    public void put(SummaryItem k, String value) {
        ...
        switch (k) {
            ...
            case Warning warn -> {
                warnings.put(wrapIdentity(warn), value);
            }
        }
    }
    ...
    public Summary putStandardPropertiesIfAbsent(StandardBundlingOperation op, Path outputDir, BundleSpec bundle) {
        ...
        switch (op) {
            ...
            default -> {
                putIfAbsent(StandardProperty.OPERATION, bundleTypeDescription(op.packageType()));
    ...
    private static String bundleTypeDescription(PackageType type) {
        return bundleTypeDescription(type, OperatingSystem.current());
    }

    private static String bundleTypeDescription(PackageType type, OperatingSystem os) {
        return I18N.getString(bundleTypeDescriptionKey(type, os));
    }

    private static String bundleTypeDescriptionKey(PackageType type, OperatingSystem os) {
        switch (type) {
    ...
    private final Map<IdentityWrapper<Warning>, String> warnings = new HashMap<>();
}
