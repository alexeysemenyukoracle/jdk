...
final class FromOptions {
    ...
    private static ApplicationLaunchers createLaunchers(Options optionValues, Function<Options, Launcher> launcherMapper) {
        ...
        var allLaunchers = new ApplicationLaunchers(mainLauncher, additionalLaunchers);

        allLaunchers.asList().stream().collect(toMap(Launcher::name, x -> x, (a, b) -> {
            throw new JPackageException(I18N.format("ERR_NoUniqueName"));
        }));
