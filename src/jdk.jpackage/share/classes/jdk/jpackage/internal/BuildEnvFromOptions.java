...
final class BuildEnvFromOptions {
    ...
    BuildEnv create(Options optionValues, Application app) {
        return create(optionValues, app, Optional.empty());
    }

    BuildEnv create(Options optionValues, Package pkg) {
        return create(optionValues, pkg.app(), Optional.of(pkg));
    }

    private BuildEnv create(Options optionValues, Application app, Optional<Package> pkg) {
        ...
        if (app.isRuntime()) {
            ...
            var path = PREDEFINED_APP_IMAGE.getFrom(optionValues);
