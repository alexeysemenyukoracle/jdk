...
final class MacFromOptions {
    ...
    private static ApplicationBuilder createApplicationBuilder(Options options) {
        ...
        if (OptionUtils.isRuntimeInstaller(options) && MacBundle.fromPath(PREDEFINED_RUNTIME_IMAGE.getFrom(options)).isPresent()) {
            // Will build a runtime bundle from the runtime bundle (not from the regular runtime directory).
            // In this case jpackage should ignore the "release" file from the runtime.
            superAppBuilder.runtimeReleaseFile(null);
        }
